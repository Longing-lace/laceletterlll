...
`;
      ...
      if (isGemini) {
    // 【核心修正】直接调用主文件中的 toGeminiRequestData 函数
    const geminiConfig = window.toGeminiRequestData(model, apiKey, systemPrompt, messages, true);
...
4. content直接写内容，不用引号包裹`;
      ...
      if (isGemini) {
    // 【核心修正】直接调用主文件中的 toGeminiRequestData 函数
    // 这个函数已经正确处理了 systemInstruction 和 messages 的分离，并兼容多密钥
    const geminiConfig = window.toGeminiRequestData(model, apiKey, systemPrompt, messages, true);
    response = await fetch(geminiConfig.url, geminiConfig.data);
  } else {
        const openAiPayload = {
